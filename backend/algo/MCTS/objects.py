...
class Vehicle:
    def __init__(self, travel_time_matrix, num=0, seats=2):
        ...
        self.capacity = seats   
        self.occupancy = 0
        self.passengers = []            # a list of current passengers
        self.passengers_assigned = []   # a list of requests assigned
        self.route = []                 # a list of locations to visit
        self.next_time = None           # next available time
    ...
    def evolve_time(self, cur_t, ev_t, next_r, travel_time_matrix):
        """
        cur_t:  the current time.
        ev_t:   a future time. 
        next_r: a list of requests.
        travel_time_matrix: currently not used. 
        """
    ...
    def check_capacity(self, r, i_p, i_d):
        """
        Returns False if capacity constraint is violated. 
        r: a request. 
        i_p: pickup index.
        i_d: drop off index.
        """
    ...
    def assign_to_vehicle(self, r, t, travel_time_matrix):
        """
        Assigns a passenger/request to the current vehicle.
        Returns True/False.
        r: a new passenger/request
        t: the current time
        travel time matrix: osm travel time estimation
        """
        # print(self.occupancy, self.capacity)
        # raise error when the capacity is exceeded. 
        # assert (self.occupancy < self.capacity)

        if self.occupancy >= self.capacity:
            return False
        
        # if there is no assigned passenger, append the request to the route plan.
        if len(self.route) == 0:
            ...
            # find the point in the route where the vehicle is closest to pick up location. 
            min_dist_p = 1e10
            ...
            # find the point in the route where the vehicle is closest to drop off location. 
            min_dist_d = 1e10
            ...
            if closest_index_d == None and closest_index_p == len(self.route)-1:
                closest_index_d = closest_index_p+1

            # check if insertion will violate capacity.
    ...
    def check_time(self, t, next_r, travel_time_matrix):
        "Checks if any vehicle has to pick up or drop off a passenger at this time."
